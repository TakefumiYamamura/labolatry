...
void generate_adaptated_mutant_vector(double** x, double** v, int i, double* cr, double* f, double* mcr, double* mf, double** x_sort, vector<vector<double> >& archive, double cp){
  int r1, r2, r3;
  cr[i] = randn(mcr[i]);
  f[i] = randc(mf[i]);
  double *xp;
...
int main()
{
  ...
  fp = fopen( "shade_with_archive.csv", "w" );
  if( fp == NULL ){
    printf( "ファイルが開けません¥n");
    return -1;
  }
  ...
  for(i=0;i<m;i++)
  {
    ...
    for (int j = 0; j < dim ; ++j)
    {
      // x[i][j] = (double)rand()/((double)RAND_MAX+1) * 100 - 100.0;
      // x[i][j] = (double)rand()/((double)RAND_MAX+1) * 200 - 100.0;
      x[i][j] = (double)rand()/((double)RAND_MAX+1) +  rand() % 201 - 100;
    }
  }
