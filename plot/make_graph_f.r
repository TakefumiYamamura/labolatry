    ...
    for (mu in 1:3){
      for (i in 1:5) {
        file_name <- paste("rastrigin/pop", populations[mu], "dim" , sep = "")
          ...
          plot(0, 0, type = "n", xlim = c(1,2000), ylim = c(-6, 2),
               xlab = "Number of generations", ylab = "rs value")
        ...
        for (k in 1:length(type)) {
          ...
          lines(xaxis, m, col = cols[k],lty = c(1,3))
          xaxis_n <- c()
          m_n <- c()
          s_n <- c()
        ...
        eps <- paste("eps/rastriginP", populations[mu], "D", sep="")
