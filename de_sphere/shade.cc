...
FILE *fp_val, *fp_val_length, *fp_std_fitness;
  ...
  sprintf(fname2, "../distance_to_centroid_de_with_archive/func%d_P%d_D%d_%dth.csv",g_function_number, g_pop_size, problem_size, g_th_num);
  // sprintf(fname2, "../csvs_length_de_with_archive_D%d/func%d_%dth.csv", problem_size, g_pop_size, g_function_number, g_th_num);
  sprintf(fname3, "../fitness_de_with_archive/func%d_P%d_D%d_%dth.csv",g_function_number, g_pop_size, problem_size, g_th_num);
  ...
  while (nfe < max_num_evaluations) {
    ...
    for (int i = 0; i < pop_size; ++i)
    {
      avg_distance_to_centroid += distance_to_centroid[i];
    }
    avg_distance_to_centroid /= pop_size;

    for (int i = 0; i < pop_size; ++i)
    {
      std_distance += pow((avg_distance_to_centroid - distance_to_centroid[i]), 2.0);
    }
    std_distance /= pop_size;
    std_distance = sqrt(std_distance);

    fprintf(fp_val_length, "%f\n", std_distance);
